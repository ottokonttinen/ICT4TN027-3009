...
Viimeisessä tehtävässä selvitetään tietokannan tyyppi ja versio. Aluksi määritellään kyselyssä palautettavien sarakkeiden määrä ja tarkistetaan, mitkä sarakkeet sisältävät tekstidataa. Tehtävä sisältää kaksi saraketta, joista molemmat sisältävät teksiä. Varmistan sen lausekkeela `'+UNION+SELECT+'eka','toka'+FROM+dual--`. Dual on taulu, joka on olemassa vain testausta varten ja palauttaa yhden rivin ja yhden sarakkeen.
